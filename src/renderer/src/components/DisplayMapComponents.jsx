...
const DisplayMapComponents = ({ componentData }) => {
  ...
  useEffect(() => {
     ...
     buttonRefs?.current[componentData.findIndex((ele)=>ele.name === activeDashboard )].classList.add("activeDashboardButton")
  ...
  <div className="w-full relative overflow-x-hidden overflow-y-scroll rounded-xl scrollbar-hide bg-lightmode-1 dark:bg-darkmode-2 h-full">
      ...
      {componentData.map((ele, index) => (
        <div
          ...
          className="w-full h-[92%] z-[3] opacity-0 absolute dashboard translate-x-[90vw] p-5 flex gap-5 flex-col"
        >
          {ele.component}
        </div>
      ))}
