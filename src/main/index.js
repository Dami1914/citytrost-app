...
import { preferenceSettings } from '@store/store.js'
...
function createWindow() {
  ...
  mainWindow.on('ready-to-show', () => {
    mainWindow.setMinimumSize(1200, mainWindow.getSize()[1])
    mainWindow.show()
  })
...
app.whenReady().then(() => {
  console.log('when ready executed')
  ...
  ipcMain.handle('create-Loan-customer', async (event, loanData, customerData) => {
    try {
      const newLoan = new Loan(loanData)
      const loanResolved = await newLoan.save()
      customerData.loans.push(loanResolved._id)
      const newCustomer = new Customer(customerData)
      const resolveCustomer = await newCustomer.save()
      console.log(resolveCustomer)
    } catch (error) {
      console.error(error)
  ...
  ipcMain.handle('toggle-theme', async (event) => {
    const currentTheme = preferenceSettings.get('theme')
    const newTheme = !currentTheme
    preferenceSettings.set('theme', newTheme)
    event.sender.send('updated-theme', newTheme) // Broadcast for future updates
